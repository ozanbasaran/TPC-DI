...
def get_finwire_row_types(finwire_file_path):
    colspecs = [(15, 18)]
    finwire_type_df = pd.read_fwf(finwire_file_path, colspecs=colspecs, header=None)
...
def parse_finwire_data(finwire_file_path):
    ...
    for row_type in row_types:
        if (row_type == 'CMP'):
            ...
            finwire_dfs['CMP']['FoundingDate'] = pd.to_datetime(finwire_dfs['CMP']['FoundingDate'], errors='coerce', format='%Y%m%d-%H%M%S')
        elif (row_type == 'SEC'):
...
def get_finwire_dfs(finwire_file_paths):
    ...
    # Inside the get_finwire_dfs function
    for file_path in finwire_file_paths:
        finwire_df_dict = parse_finwire_data(file_path)

        for finwire_type in finwire_df_dict.keys():
            if (finwire_type == 'CMP'):
                cmp_df = cmp_df._append(finwire_df_dict['CMP'])
            elif (finwire_type == 'SEC'):
                sec_df = sec_df._append(finwire_df_dict['SEC'])
            elif (finwire_type == 'FIN'):
                fin_df = fin_df._append(finwire_df_dict['FIN'])

    return {'CMP': cmp_df, 'SEC': sec_df, 'FIN': fin_df}


# Define file paths
root_path = r'C:\Users\ozanb\OneDrive\Masaüstü\scale5'
finwire_source_files_path = root_path + r'\Batch1'
finwire_schema_files_path = root_path + r'\Schemas'
...
finwire_files = [finwire_source_files_path + '\\' + file for file in os.listdir(finwire_source_files_path) if
                 re.search(r'^FINWIRE......$', file)]
g = get_finwire_dfs(finwire_files)

g['CMP']['FoundingDate'] = pd.to_datetime(g['CMP']['FoundingDate'], errors='coerce')
g['SEC']['FirstTradeDate'] = pd.to_datetime(g['SEC']['FirstTradeDate'], format='%Y%m%d', errors='coerce')
g['SEC']['FirstTradeExchg'] = pd.to_datetime(g['SEC']['FirstTradeExchg'], format='%Y%m%d', errors='coerce')
g['FIN']['QtrStartDate'] = pd.to_datetime(g['FIN']['QtrStartDate'],errors='coerce')
g['FIN']['PostingDate'] = pd.to_datetime(g['FIN']['PostingDate'],errors='coerce')
